...
EOT
    ...
    # 
    #  singletonでない場合は，p_cellcbを内部関数に渡す
    #  呼び口関数を内部関数から呼び出せるようにするため
    #
    arg_p_cellcb = ""
    argv_p_cellcb = ""
    if ! b_singleton then
        arg_p_cellcb = "p_cellcb"
        argv_p_cellcb = "#{ct_name}_CB *p_cellcb"
    end
    #
    ...
    signature.get_function_head_array.each { |f|
      ...
      file.print "        if( tTransparentUnmarshaler_#{@signature.get_name}_#{f_name}(#{arg_p_cellcb}) != E_OK )\n"
      file.print "            goto error_reset;\n"
      file.print "        break;\n"
...
=end
        ...
        if bit_size.nil?
            raise "HRP RPC supports only specified bit_size"
        else
            case bit_size
            when 8, 16, 32, 64, 128
            else
...
=end
  ...
  def gen_preamble file, b_singleton, ct_name, global_name
    ...
    @signature.get_function_head_array.each { |f|
      ...
      f_type = f.get_declarator.get_type
      id = @signature.get_id_from_func_name( f_name )
      file.print "static ER  tTransparentUnmarshaler_#{@signature.get_name}_#{f_name}(#{argv_p_cellcb});\t/* func_id: #{id} */\n"
    }
    file.print "\n"
...
EOT
      ...
      file.print "tTransparentUnmarshaler_#{@signature.get_name}_#{f_name}(#{argv_p_cellcb})\n"
      file.print "{\n"
      file.print "	ER  ercd_;\n"
